    ...
    <div class="scroll-content">
      ...
      >
        <div class="pullup-wrapper" v-if="pullUpLoad">
          <div class="before-trigger" v-if="true">
            <span>{{pullUpTxt}}  </span>
          </div>
...
<script type="text/ecmascript-6">
  ...
  import loading from './scroll/loading'
  import {getRect} from './scroll/dom'
  export default {
    data() {
      return {
        bubbleY: 0,
    ...
    computed: {
      pullUpTxt() {
        ...
        return this.hasNext ? moreTxt : noMoreTxt
      },
      refreshTxt() {
    ...
    created() {
      if(this.pullDownRefresh)this.$nextTick(()=>{
    ...
    methods: {
      ...
      _initScroll() {
        ...
        if(this.scroll.scrollerHeight === this.scroll.wrapperHeight){
          document.querySelector(".list-wrapper").style.height = window.innerHeight + 'px';
        }
        this.scroll = new BScroll(this.$refs.wrapper, options);
        // 监听滚动
